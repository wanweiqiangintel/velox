 ...
 public:
  explicit ParquetReaderBenchmark(bool disableDictionary)
    ...
    if (disableDictionary_) {
      ...
      } else if (FLAGS_compression.compare("zstd") == 0) {
  ...
  void writeToFile(
    ...
    // for (auto& batch : batches) {
      ...
      if (FLAGS_table_name.compare("part") == 0) {
        rowVector1 = facebook::velox::tpch::genTpchPart(200000, 0, 10);
        for (int i = 0; i < 10; i++) {
          std::cout << "i: " << i << ", num row: " << rowVector1->size() << std::endl;
          writer_->write(rowVector1);
        }
      } else if (FLAGS_table_name.compare("region") == 0) {
        rowVector1 = facebook::velox::tpch::genTpchRegion(5, 0, 10);
          writer_->write(rowVector1);
      } else if (FLAGS_table_name.compare("partsupp") == 0) {
        rowVector1 = facebook::velox::tpch::genTpchPartSupp(800000, 0, 10);
        for (int i = 0; i < 10; i++) {
          std::cout << "i: " << i << ", num row: " << rowVector1->size() << std::endl;
          writer_->write(rowVector1);
        }        
      } else if (FLAGS_table_name.compare("supplier") == 0) {
        rowVector1 = facebook::velox::tpch::genTpchSupplier(100000, 0, 10);
        writer_->write(rowVector1);
        ...
        rowVector1 = facebook::velox::tpch::genTpchOrders(150000, 0, 10);
        for (int i = 0; i < 20; i++) {
          std::cout << "i: " << i << ", num row: " << rowVector1->size() << std::endl;
          writer_->write(rowVector1);
        }             
      } else if (FLAGS_table_name.compare("customer") == 0)  {
        rowVector1 = facebook::velox::tpch::genTpchCustomer(150000);
        for (int i = 0; i < 10; i++) {
          std::cout << "i: " << i << ", num row: " << rowVector1->size() << std::endl;
          writer_->write(rowVector1);
        }           
      } else if (FLAGS_table_name.compare("lineitem") == 0)  {
        rowVector1 = facebook::velox::tpch::genTpchLineItem(120000);
        for (int i = 0; i < 10; i++) {
          std::cout << "i: " << i << ", num row: " << rowVector1->size() << std::endl;
          writer_->write(rowVector1);
        }            
